...
public class GarlicRecipes {
	...
	private void fixGarlicArmour() {
		...
		for (int i = 0; i < hunterItemsSilvered.length; i++) {
			Utils.removeAllCraftingRecipesByOutputItem(new ItemStack(hunterItemsSilvered[i][1]));
	...
	private void fixVampireBook() {
		Utils.removeAllCraftingRecipesByOutputItem(new ItemStack(Witchery.Items.VAMPIRE_BOOK));
		for (ItemStack garlic : mGarlics) {
