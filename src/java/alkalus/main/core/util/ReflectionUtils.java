...
public class ReflectionUtils {
	...
	public static boolean dynamicallyLoadClassesInPackage(String aPackageName) {
		...
		try {
			...
			for (ClassPath.ClassInfo info : path.getTopLevelClassesRecursive(aPackageName)) {
				...
				if (clazz != null) {
					loaded++;
					Logger.REFLECTION("Found "+clazz.getCanonicalName()+". ["+loaded+"]");
				}
			}
	...
	public static Object invokeNonVoid(Object aInstance, Method aMethod){
		return invokeNonVoid(aInstance, aMethod, new Object[] {});
	}
	
	public static Object invokeNonVoid(Object aInstance, Method aMethod, Object[] aValues){		
		if (aMethod == null || (!ReflectionUtils.isStaticMethod(aMethod) && aInstance == null) || aValues == null){
			return false;
		}
		try {
			return aMethod.invoke(aInstance, aValues);
		}
		catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
			Logger.REFLECTION("Failed to Dynamically invoke "+aMethod.getName()+" on an object of type: "+aInstance.getClass().getName());
			e.printStackTrace();
		}
		Logger.REFLECTION("Invoke failed or did something wrong.");		
		return null;
	...
	private static void dumpClassInfo(Class<?> aClass) {
		Logger.REFLECTION("We ran into an error processing reflection in "+aClass.getName()+", dumping all data for debugging.");	
		// Get the methods
		...
		for (Constructor<?> c : consts) {
			System.out.println(c.getName()+" | "+c.getParameterCount()+" | "+getDataStringFromArray(c.getParameterTypes()));
		}
	}
